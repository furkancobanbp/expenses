...
class AppController:
    ...
    def get_forecasts_by_month(self, year: int, month: int) -> List:
    ...
    # Forecast methods
    def add_forecast(self, name: str, amount: float, category_name: str, date: datetime) -> bool:
        """Add a new forecast transaction"""
        try:
            # Get the transaction type from the category
            category_type = self.category_manager.get_category_type(category_name)
            ...
            self.forecast_manager.add_forecast(name, amount, category_name, category_type, date)
            return True
        except Exception as e:
            print(f"Error adding forecast: {e}")
            return False
    ...
    def get_monthly_forecast_summary(self, year: int, month: int) -> Dict:
        """Get summary of forecasted income and expenses for a month"""
        return self.forecast_manager.get_monthly_forecast_summary(year, month)
    
    def get_forecast_vs_actual_comparison(self, year: int, month: int) -> Dict:
        """Get comparison between forecasted and actual data for a month"""
        actual_data = self.get_monthly_summary(year, month)
        return self.forecast_manager.get_comparison_data(actual_data, year, month)
    
    def get_category_comparison(self, year: int, month: int) -> Dict:
        """Get category-wise comparison between forecasted and actual data"""
        actual_transactions = self.finance_manager.get_transactions_by_month(year, month)
        return self.forecast_manager.get_category_comparison(actual_transactions, year, month)
    ...
    def mark_forecast_realized(self, forecast_id: str, transaction_id: str) -> bool:
        ...
        return self.forecast_manager.mark_forecast_realized(forecast_id, transaction_id)
    ...
    def auto_match_transactions_to_forecasts(self) -> tuple:
        """Match actual transactions to forecasts and mark them as realized"""
        try:
            actual_transactions = self.finance_manager.get_all_transactions()
            return self.forecast_manager.check_realization_against_actual(actual_transactions)
        except Exception as e:
            print(f"Error matching transactions to forecasts: {e}")
            return (0, 0)